...
from path_model.model_transformer import LayerNormFp32, LayerNorm, QuickGELU, Attention, VisionTransformer, TextTransformer
from path_model.knowledge_bert import PATH_BERT
...
class PKCLIP(nn.Module):
    ...
    def __init__(
        ...
        self.knowledge = PATH_BERT(bert_model_name = bert_pretrain, feature_dim = embed_dim)
    ...
    def encode_knowledge(self, text, normalize: bool = False):
        x = self.knowledge(text)
        return F.normalize(x, dim=-1) if normalize else x
    ...
    def forward(
        ...
        text_clip = text['text_clip']
        text_knowledge = text['text_knowledge']
        image_features = self.encode_image(image, normalize=True) if image is not None else None
        text_features = self.encode_text(text_clip, normalize=True) if text is not None else None
        knowledge_features = self.encode_knowledge(text_knowledge, normalize=True) if text is not None else None
        if self.output_dict:
            return {
                "image_features": image_features,
                "text_features": text_features,
        ...
        return image_features, text_features, knowledge_features, self.logit_scale.exp()
    
class PathCLIP(nn.Module):
    output_dict: torch.jit.Final[bool]
    ...
    def __init__(
        ...
        self.text = PATH_BERT(bert_model_name = bert_pretrain, feature_dim = embed_dim)
...
class KEP(nn.Module):
    ...
    def forward(
        ...
        image_features = self.encode_image(image, normalize=True) if image is not None else None
        text_features = self.encode_text(text, normalize=True) if text is not None else None
        knowledge_features = self.encode_knowledge(text, normalize=True) if text is not None else None
        if self.output_dict:
            return {
                "image_features": image_features,
                "text_features": text_features,
