...
def get_tokenizer(text,tokenizer,max_length, ismask = True):
    try:
        token_list = tokenizer(list(text),add_special_tokens=True,max_length=max_length,pad_to_max_length=True,return_tensors='pt')
    except:
        token_list = tokenizer(list(text))
    if ismask and random.random() < 0.5:
        mask_text, _ = mask_tokens(token_list['input_ids'],tokenizer)
        token_list['input_ids'] = mask_text
    return token_list


def train_one_epoch(model,tokenizer, dataloader,num_batches,num_samples, loss_fn, epoch, optimizer, scaler, scheduler, args, tb_writer=None):
    ...
    for i, (input_text, did, tid, attr) in enumerate(dataloader):
        # print(batch)
        step = num_batches_per_epoch * epoch + i
        scheduler(step)
        input_text = get_tokenizer(input_text,tokenizer,max_length=args.max_length, ismask=False).to(device=device)
        ...
        with autocast():
            ...
            total_loss = loss_fn['metric_loss'](text_features, target)
        ...
        if is_master(args) and (i % 100 == 0 or batch_count == num_batches_per_epoch):
            ...
            logging.info(
                f"Train Epoch: {epoch} [{num_samples:>{batch_count}}/{num_batches_per_epoch} ({percent_complete:.0f}%)] "
